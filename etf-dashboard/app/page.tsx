      ...
      <section className="max-w-6xl mx-auto px-6 py-20 border-t border-[#1f2937]">
        ...
        <div className="max-w-2xl mx-auto space-y-4">
          <ChangelogEntry
            date="May 29, 2026"
            tag="bugfix"
            title="Streaks and 'what changed' now ignore stale data — including the weekend"
            desc={"Started with a simple question — does the streak tracker count over the weekend? — and pulled a bigger thread. Two problems, same root cause. (1) Weekends and holidays: the scraper sometimes grabs a Saturday snapshot, which is just Friday's holdings copied forward — nothing traded. The old code saw that flat day and called the streak dead, chopping a two-week accumulation run down to 'started Monday.' (2) The sneakier one: not every fund posts on the same schedule, and the real as-of date lives *inside* the file, not in the filename. Some providers serve a frozen snapshot for weeks (one fund's holdings were stuck back in January) while the weights still wiggle as prices get recalculated — so our 'what changed today' panels were showing phantom buys and sells on positions nobody had touched. Fixed both by trusting the provider's own as-of date: a move only counts if that date actually advanced or the share count changed (a real share change always counts, even on a stale date). Funds that don't stamp a date — Avantis, the YieldMax/Kurv/EGG income funds — fall back to the old weight check, which is safe because a real stock's weight never lands identical two trading days running. Streaks now span weekends and holidays, the Position Sizing / Changes panels stopped inventing trades, and a pile of genuine multi-week conviction runs that were hidden are back. One more bit of cleanup: streaks now ignore the cash bucket — money-market funds (that First American sweep nearly every fund parks cash in) and the 'Cash & Other' line. Those drift in weight as the rest of the book moves, so they were showing fake 'conviction' runs on what is literally idle cash. A streak should mean someone's actually building a position. (And no, a real stock that happens to trade under the ticker CASH — Pathward Financial — still counts.)"}
          />
          <ChangelogEntry
            date="May 26, 2026"
            tag="feature"
