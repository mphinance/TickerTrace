// Patch Notes from the Trenches — the full changelog history for the
// landing page (see CLAUDE.md's "Patch Notes from the Trenches" standing
// instruction).
//
// Newest first. To add an entry: prepend a new object to CHANGELOG_ENTRIES
// below — do NOT add JSX to app/page.tsx, which now just imports and renders
// this array via <ChangelogList /> (components/changelog-list.tsx).
//
// This file was generated from the 165 <ChangelogEntry /> elements that used
// to live inline in app/page.tsx (one per shipped change since Feb 2026) —
// content is unchanged, only the format moved.

export type ChangelogTag = 'feature' | 'bugfix' | 'housekeeping' | 'polish';
...
export const CHANGELOG_ENTRIES: ChangelogEntryData[] = [
  {
    date: "September 5, 2026",
    tag: "bugfix",
    title: "The site wasn't actually installable as an app, and this changelog was 97 phone-screens tall",
    desc: "Took a real pass at mobile instead of eyeballing it. Four things were wrong. The manifest and real app icons we shipped a while back were doing nothing this whole time — Chrome and Android also require a registered service worker before they'll offer 'Install app', and we never added one, so the install prompt just never fired for anyone. It's there now, and deliberately dumb: it caches the static shell so an offline reopen shows something real, and it does not touch API responses, because a service worker quietly serving yesterday's institutional holdings would be worse than no offline support at all. Second: this changelog — the thing you're reading right now — used to be 165 entries typed straight into the landing page and rendered all at once, which made the page 79,000 pixels tall on a phone before you ever scrolled past it. It's a data file now, loads collapsed to the 10 most recent, and there's a button for the rest. Third: the landing nav was overflowing the screen at normal phone widths — the 'Open Intel' button, the main call to action on the page every link on the internet points at, was rendering 140px off the right edge of the viewport where nobody could tap it. Fourth: five search and chat inputs across the site (the ticker search, the Changes search, the Ask TickerTrace key/model/chat fields) were under 16px, which is the exact size that makes iOS Safari zoom in on tap and then never zoom back out. All fixed.",
  },
  {
